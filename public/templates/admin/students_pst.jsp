...
<div class="panel-body" ng-controller="VerificationProvostCtrl">
    <div class="row">
        <div class="col-xs-12">  
            <form class="form-inline text-center" role="form" ng-submit="findUnverified()">
                <div class="form-group">
                    <select class="form-control input-sm" ng-model="forUnverified.dep" ng-options="x for x in distinct_deps | orderBy: 1" required="true">
            ...
            <div ng-if="unverified.length > 0" class="alert alert-warning">
                <strong>Important Note for Provost Office</strong>: Kindly don't verify any request if it is not as per your record. If you found
            ...
            <div ng-if="unverified.length > 0" class="pull-right">
                <button class="btn btn-success btn-xs" ng-csv="getUnverified" csv-header="getHeader()" filename="provost_unverified.csv">
            ...
            <div id="printarea" hidden="true">
                <p><strong>Important Note for Provost Office</strong>: Kindly don't verify any request if it is not as per your record. If you found
                    any student's information not matching as per your record leave as it is unverified. In case any students whose records are 
                    not matching with yours and insisting you to verify, ask him/her to edit their profile and provide correct information.
                    Every student can edit/modify their information until you verify it.</p>
                <p>
                <center><strong>Department: {{forUnverified.dep.name}}, Hall: {{hall}}</strong></center>
                </p>
                <table border="1">
                    <tr>
                        <th>SR#</th>                    
                        ...
                        <th>Course</th>
                        <th>Sem/Year</th>
                        <th ng-if="user.label === 'Provost N.R.S.C. Hall'">Address</th>
                    ...
                    <tr ng-repeat="student in unverified| orderBy: 'fullName'">                        
                        ...
                        <td>{{student.enroll| uppercase}}</td>
                        <td>{{student.profile.facultyNumber| uppercase}}</td>
                        <td>{{student.profile.courseName| uppercase}}</td>
                        <td>{{student.profile.semesterYear}}</td>    
                        <td ng-if="user.label === 'Provost N.R.S.C. Hall'">{{student.profile.nrscadd}}</td>
                        <td></td>
                        <td></td>
                    </tr>                    
                </table>
            ...
            <table class="table table-striped" ng-if="unverified.length > 0">
                <tr>
                    <th>SR#</th>
                    <th>
                        <input type="checkbox" ng-model="provost.value" ng-click="provostAll()">                                        
                    </th>                    
                    <th>Name of Students</th>
                    <th>Enrollment No</th>
                    <th>Faculty No</th>
